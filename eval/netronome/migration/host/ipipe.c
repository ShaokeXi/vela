...
static float nic_input_rate = 15;

static void ipipe_lcore_stats()
{
			...
			switch (opt)
			{
				case PKT_INIT:
					...
					if (flow_ret == -ENOENT)
					{
						...
						{
							/* Assign the NAT entry to one state group */
							int grpIdx = ev.nic.bucket & (grpNum - 1);
							stateGrp[grpIdx].states[stateGrp[grpIdx].cnt] = flow_ret;
							stateGrp[grpIdx].cnt++;

							rte_atomic32_inc(&mCtrl.totalFlowCnt);
							flowStore[flow_ret] = ev;
							if (stateGrp[grpIdx].status == UNINITIATED)
							{
								float random_value = (float)rand() / RAND_MAX;
								if (random_value <= ratio && rte_atomic32_read(&mCtrl.nicGrpCnt) <= mCtrl.nicGrpSize)
								{
									/* Assign the group to NIC */
									rte_atomic32_inc(&mCtrl.nicGrpCnt);
									stateGrp[grpIdx].status = INVALID;
									flowStore[flow_ret].status = INITIAL_INVALID;
								}
								else
								{
									/* Assign the group to CPU */
									rte_atomic32_inc(&mCtrl.cpuGrpCnt);
									stateGrp[grpIdx].status = VALID;
									flowStore[flow_ret].status = INITIAL_VALID;
								}
							}
							else if (stateGrp[grpIdx].status == INVALID)
							{
								flowStore[flow_ret].status = INITIAL_INVALID;
							}
							else if (stateGrp[grpIdx].status == VALID)
							{
								flowStore[flow_ret].status = INITIAL_VALID;
							}
...
int ctrl_loop_ipipe(void)
{
	...
	int evict_error = 0, evict_times = 0, evict_loop = 0;
	...
	while (!force_quit)
    {
		...
		{
			if (rte_atomic32_read(&mCtrl.nicGrpCnt) < mCtrl.nicGrpSize)
			{
				/* Offload one state group at a time */
				int i;
				for (i = 0; i < grpNum; i++)
				{
					if (stateGrp[i].status == VALID)
					{
						int j, invalid_cnt = 0;
						for (j = 0; j < stateGrp[i].cnt; j++)
						{
							int fid = stateGrp[i].states[j];
							if (flowStore[fid].opt && flowStore[fid].status != INVALID)
							{
								/* Sanity check */
								flow_ret = rte_hash_lookup(flowTbl, &flowStore[fid].addr);
								if (flow_ret != fid)
								{
									data_error++;
									flowStore[fid].opt = 0;
									flowStore[fid].status = INVALID;
								}
								else
								{
									elem_ret = rte_hash_lookup(elemTbl, &flowStore[fid].nic);
									if (elem_ret < 0)
									{
										if ((elem_ret = rte_hash_add_key(elemTbl, (void const *)&flowStore[fid].nic)) >= 0)
										{
											elemMap[elem_ret] = fid;
										}
									}
									else if (elemMap[elem_ret] != fid)
									{
										data_error++;
										elemMap[elem_ret] = fid;
									}
								...
								info[BUCKET] = rte_cpu_to_be_32(flowStore[fid].nic.bucket);
								info[INDEX] = rte_cpu_to_be_32(flowStore[fid].nic.index);
								send_ctrl_pkt(ctrlPrt, intOff, info);
								del_times++;
							}
							else if (flowStore[fid].status == INVALID || flowStore[fid].opt == 0)
							{
								invalid_cnt++;
							}
							else
						...
						if (invalid_cnt == stateGrp[i].cnt)
						{
							rte_atomic32_inc(&mCtrl.nicGrpCnt);
							rte_atomic32_dec(&mCtrl.cpuGrpCnt);
							stateGrp[i].status = INVALID;
						}
						break;
					}
				}
		...
		{
			/* Evict one state group at a time */
			int i;
			for (i = 0; i < grpNum; i++)
			{
				if (stateGrp[i].status == INVALID)
				{
					int j, valid_cnt = 0;
					for (j = 0; j < stateGrp[i].cnt; j++)
					{
						int fid = stateGrp[i].states[j];
						if (flowStore[fid].opt && flowStore[fid].status != VALID)
						{
							/* Sanity check */
							flow_ret = rte_hash_lookup(flowTbl, &flowStore[fid].addr);
							if (flow_ret != fid)
						...
						else if (flowStore[fid].status == VALID || flowStore[fid].opt == 0)
						{
							valid_cnt++;
						}
						else
					...
					if (valid_cnt == stateGrp[i].cnt)
					{
						rte_atomic32_inc(&mCtrl.cpuGrpCnt);
						rte_atomic32_dec(&mCtrl.nicGrpCnt);
						stateGrp[i].status = VALID;
					}
					break;
				}
			}
        ...
        if (diff_ts > interval)
		{
			...
			if (file != NULL)
			{
				fscanf(file, "%f %f", &nic_busy_threshold, &nic_input_rate);
				fclose(file);
				printf("nic busy threshold: %f, nic input rate: %f\n", nic_busy_threshold, nic_input_rate);
			}
			
			int k;
			if (idle_avg < nic_busy_threshold || nic_rate < nic_input_rate)
			{
			...
			// else if (nic_idle > NIC_IDLE_THRESHOLD)
			// {
			// 	if (nic_flag != AVAILABLE)
			// 	{
			// 		info[FLAG] = rte_cpu_to_be_32(AVAILABLE);
			...
			RTE_LOG(INFO, USER1, "[CPU %d] new %d states, del %d states, cpu %d groups, cpu %d states, nic %d groups, nic %d states, total %d states, "
					"recv %d pps, evict %d pps, del %d pps, del error %d, data_error %d, flow_error %d, evict_loop %d\n", 
					core, new_state, del_state, rte_atomic32_read(&mCtrl.cpuGrpCnt), rte_atomic32_read(&mCtrl.cpuFlowCnt),
					rte_atomic32_read(&mCtrl.nicGrpCnt), rte_atomic32_read(&mCtrl.nicFlowCnt), rte_atomic32_read(&mCtrl.totalFlowCnt), 
					recv_pkts, evict_times, del_times, del_error, data_error, flow_error, evict_loop);
			RTE_LOG(INFO, USER1, "[CPU %d] CPU %.2f Mpps, NIC %.2f Mpps, ME idle time %.2f us, NIC status %s, update success %d, failure %d, "
					...
					core, cpu_rate, nic_rate, idle_avg, nic_status_map[nic_flag], update_success, update_failure,
					cpu_heavy, cpu_light, nic_heavy, nic_light);
			prev_tv = cur_tv;
			pre_nfd_lost = cur_nfd_lost;
			new_state = evict_times = del_state = del_times = evict_loop = 0;
