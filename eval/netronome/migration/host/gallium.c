...
static void gallium_lcore_stats()
{
...
int ctrl_loop_gallium(void)
{
	...
	int elem_ret;
	int evict_error = 0;
	int rescue_offload = 0, rescue_evict = 0;
	...
	while (!force_quit)
    {
		...
		int tmp_rescue_offload = 0, tmp_rescue_evict = 0;
		for (i = 0; i < FLOW_NUM; i++)
		{
			if (flowStore[i].status == INITIAL_INVALID)
			{
				int flow_ret = rte_hash_lookup(flowTbl, &flowStore[i].addr);
				if (flow_ret != i)
				{
					data_error++;
					flowStore[i].opt = 0;
					flowStore[i].status = INVALID;
				}
				else
				{
					elem_ret = rte_hash_lookup(elemTbl, &flowStore[i].nic);
					if (elem_ret < 0)
					{
						if ((elem_ret = rte_hash_add_key(elemTbl, (void const *)&flowStore[i].nic)) >= 0)
						{
							elemMap[elem_ret] = i;
						}
					}
					else if (elemMap[elem_ret] != i)
					{
						data_error++;
						elemMap[elem_ret] = i;
					}
			...
			else if (flowStore[i].status == INITIAL_VALID)
			{
			...
			if (tmp_rescue_offload >= BURST_SIZE || tmp_rescue_evict >= BURST_SIZE)
			{
				rescue_offload += tmp_rescue_offload;
				rescue_evict += tmp_rescue_evict;
				break;
			}
		}
        ...
        if (diff_ts > interval)
		{
			...
			double nic_rate = nic_pkts / nic_interval / 1000.0;
			prtMon[ctrlPrt].nic_last_count = prtMon[ctrlPrt].nic_cur_count;
			...
			RTE_LOG(INFO, USER1, "[CPU %d] new %d states, del %d states, cpu %d states, nic %d states, total %d states, "
					"recv %d pps, evict %d pps, del error %d, data_error %d, flow_error %d, rescue_offload %d, rescue_evict %d\n", 
					core, new_state, del_state, cpu_state, rte_atomic32_read(&mCtrl.nicFlowCnt), rte_atomic32_read(&mCtrl.totalFlowCnt), 
					recv_pkts, del_times, del_error, data_error, flow_error, rescue_offload, rescue_evict);
			RTE_LOG(INFO, USER1, "[CPU %d] CPU %.2f Mpps, NIC %.2f Mpps, ME idle time %.2f us, NIC status %s, update success %d, failure %d\n",
					core, cpu_rate, nic_rate, nic_idle, nic_status_map[nic_flag], update_success, update_failure);
			prev_tv = cur_tv;
			pre_nfd_lost = cur_nfd_lost;
			new_state = del_state = del_times = 0;
			update_success = update_failure = 0;
			recv_pkts = 0;
			rescue_offload = rescue_evict = 0;
        }
	}
